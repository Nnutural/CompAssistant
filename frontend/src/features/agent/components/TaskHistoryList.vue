<template>
  <section class="panel-card" data-testid="task-history-card">
    ...
    <div class="filters-row">
      ...
      <label class="filter-field">
        <span>任务类型</span>
        <select :value="taskTypeFilter" data-testid="history-task-type-filter" @change="updateTaskTypeFilter">
    ...
    <div v-if="!items.length" class="empty-state">当前筛选条件下没有历史任务。</div>
...
import { AGENT_TASK_OPTIONS, HISTORY_STATUS_OPTIONS, STATUS_LABELS, TASK_TYPE_LABELS } from '../config'
...
function updateStatusFilter(event: Event) {
  emit('change-status', (event.target as HTMLSelectElement).value)
}

function updateTaskTypeFilter(event: Event) {
  emit('change-task-type', (event.target as HTMLSelectElement).value)
}
...
.refresh-btn,.secondary-btn { border:none; border-radius:10px; padding:8px 12px; background:#e8e8ed; color:#1d1d1f; cursor:pointer; }
.refresh-btn:disabled,.secondary-btn:disabled { opacity:.6; cursor:default; }
.filters-row { margin-top:18px; display:grid; grid-template-columns:repeat(2, minmax(0,1fr)); gap:12px; }
.filter-field { display:flex; flex-direction:column; gap:6px; }
.filter-field span { font-size:12px; font-weight:600; color:#6e6e73; }
.filter-field select { border:1px solid #d2d2d7; border-radius:10px; background:#f5f5f7; padding:10px 12px; color:#1d1d1f; }
.empty-state { margin-top:16px; color:#6e6e73; }
.history-list { margin-top:18px; display:flex; flex-direction:column; gap:12px; }
.history-item { border:1px solid #e8e8ed; border-radius:12px; background:#f8f8fa; padding:14px; }
.history-item.active { border-color:#0071e3; box-shadow:0 0 0 3px rgba(0,113,227,.12); }
.history-top { display:flex; justify-content:space-between; gap:12px; }
.history-top p { margin-top:4px; color:#6e6e73; font-size:12px; }
.history-summary { margin-top:10px; color:#1d1d1f; line-height:1.5; }
.history-meta { margin-top:10px; display:flex; flex-wrap:wrap; gap:10px; color:#6e6e73; font-size:12px; }
.history-actions { margin-top:12px; display:flex; justify-content:flex-end; }
.load-more-row { margin-top:16px; display:flex; justify-content:center; }
