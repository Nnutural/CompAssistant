<template>
  <section class="panel-card" data-testid="artifact-panel-card">
    ...
    <div v-if="!runStatus" class="empty-state">请先选择或创建任务。</div>
    <div
      v-else-if="!items.length && !['completed', 'awaiting_review', 'cancelled'].includes(runStatus.status)"
      class="empty-state"
    >
      任务结束后会在这里显示结构化结果。
    </div>
    <div v-else-if="!items.length" class="warning-state">当前终态任务没有可展示的结构化产物。</div>

    <div v-else class="artifact-list" data-testid="artifact-list">
      <article v-for="item in items" :key="item.artifact_id" class="artifact-card" data-testid="artifact-item">
...
.empty-state,.warning-state { margin-top:16px; color:#6e6e73; }
.warning-state { color:#915f00; }
.artifact-list { margin-top:18px; display:flex; flex-direction:column; gap:14px; }
.artifact-card { border:1px solid #e8e8ed; border-radius:12px; padding:16px; background:#f5f5f7; }
.artifact-header h4 { font-size:16px; color:#1d1d1f; }
.artifact-header span { display:inline-block; margin-top:4px; font-size:12px; color:#6e6e73; text-transform:uppercase; letter-spacing:.05em; }
.artifact-card pre { margin-top:12px; white-space:pre-wrap; word-break:break-word; font-size:13px; line-height:1.5; color:#1d1d1f; }
</style>
